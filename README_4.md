...
|------|-------------|---------|
| Captain | Persuasion | Command Bridge |
| Engineer | Arcana | Reactor Status |
| Pilot | Acrobatics | Navigation Bridge |
| Gunner | Sleight of Hand | Gunnery Deck |
...
The Captain tab manages damage control, sensor operations, and the **Standing Orders** deck - a hand of tactical order cards drawn each round.

### Initiative

Click the d20 icon to roll ship initiative for the combat tracker: `1d20 + Persuasion modifier`. The result is stored directly in the combat tracker.

### Command Allocation

Roll Persuasion once per round. Allocate the Points to:
...
The Captain also operates the **Sensor Radar**: an interactive canvas overlay plotting all ship tokens as blips. The primary task is building and maintaining **Sensor Locks** on enemy contacts.
...
| 1 | Active Ping | 2 | Name, bearing, and ship class; enables Gunner targeting |
| 2 | Breach Analysis | 4 | Shield percentages per sector |
| 3 | Deep Scan | 7 | Armor, shields, hull, and fire arcs per sector; reveals actual ship name |
| 4 | Targeting Solution | 10 | +2 to hit for the Gunner; reveals active conditions and defenses (immunities, resistances, vulnerabilities) |

Locks decay by one tier each round unless refreshed.

> **Autoscan bonus:** Targets within the auto-scan range are automatically locked at Tier 2.

### Battle Damage Assessment (Post-Fire)

After the Gunner fires, the Captain may perform one BDA correction (rolled with Persuasion):

| Correction | Effect |
|-----------|--------|
| Adjust Bearing | +2 to hit on the next attack against this target |
| Target Weak Point | +Points armor penetration on the next attack |
| Fire for Effect | Expands the crit range by Points (2 Points → crit on natural 18–20) and lowers the crit severity threshold |
...
| Action | Effect |
|--------|--------|
| Cooling Protocol | Spend AP, roll Arcana; vent heat equal to AP spent + Points (minimum 1) |
| Emergency Vent | Instantly clear all heat; start Internal Fire equal to heat vented; locks core distribution next round |
| Hull Repair | Spend AP, roll Arcana; restore HP equal to AP spent + Points (minimum 1); costs +1 heat per HP; blocked while internal fire is active |
| Suppress Fire | Spend AP, roll Arcana; reduce fire by AP spent + Points (minimum 1) |
...
| Track | Effect per Point |
|-------|------------------|
| Speed | +1 unit of movement this round |
| Maneuverability | +1 degree of bearing change this round |
| Evasion | −1 to all incoming attack rolls this round (equivalent to +1 AC) |
...
- The ramming ship soaks incoming damage with its bow armor
- Ram damage is bludgeoning; each ship's immunities/resistances/vulnerabilities apply
...
The Gunner fires the ship's weapon batteries and manages crew logistics and ordnance deployment. Hostiles can only be targeted and damage can only be assessed with the assistance of the Captain.

### Gunnery Allocation

Roll Sleight of Hand once per round. Allocate Points to:

| Track | Effect per Point |
|-------|------------------|
| Accuracy | +1 to hit |
| Penetration | +1 armor penetration |
