# Causodes's Ship Combat (D&D 5e) Setup Guide
...
All three are created through a single **Starship** entry in the Create Actor dialog — picking it opens a follow-up prompt to choose **Player Starship**, **NPC Starship**, or **Ordnance**.
...
## Player Ship
To set up a **player ship**, create a new **Starship** actor, choose **Player Starship** in the follow-up prompt, and grant all players **Observer** ownership permissions.

You will see a sheet like the one below:

![](https://github.com/user-attachments/assets/f3221e70-5120-4717-aa48-a083a2e918bd)

### Header & Sidebar
Click the edit button in the top left corner and let's get started!

Let's start with the core stats in the header card and sidebar and what each of the sections do:
* **Hit Points**
    * Current HP/Max HP: The current/maximum hitpoints of the player ship. The max HP value is **NOT** inherited from ship components and is manually set (via the native dnd5e hit points config).
        * The value can be overriden by the GM by double clicking it and editing it.
    * Internal Fire: Fire deals damage directly to a ship's HP pool, bypassing Armor and Shields. 
        * Internal Fire will originate from various sources and can be dealt with using certain crew abilities. 
        * The value can be overriden by the GM by double clicking it and editing it.
* **Initiative**
    * Initiative: Used to determine the starship's combat initiative. This uses the actor in the `Captain` role's stats and defaults to the Captain role's main skill (Persuasion by default).
* **Armor Class**
    * AC: The Armor Class of the ship is used in calculating whether an attack against it hits the ship. This value is inherited from a combination of the ship's `Engine` and `Armor` **Ship Components**. 
        * A breakdown of the contribution can be viewed by opening the AC configuration, and flat modifiers can be injected there as well.
    * Armor: The Armor of the ship decreases all incoming damage to the ship's respective quadrant by a flat modifier equal to its value. This value is inherited from the ship's `Armor` **Ship Component**. 
        * Armor values can be damaged/reduced by various sources.
* **Shields**
    * Shields: Each point of shield will completely negate the damage of one successful incoming attack to its respective quadrant, draining one point of shield instead. The maximum shield value per quadrant is inherited from the ship's `Shields` **Ship Component**. 
...
* **Damage Immunities, Resistances, Vulnerabilities, and Modification**
    * These use the native dnd5e trait sets, edited from the sidebar cog buttons exactly like a regular NPC, and follow standard 5e semantics:
        * **Immunity**: The starship takes no damage of this type. Attacks of this type **DO NOT** drain Shields.
        * **Resistance**: Incoming damage of this type is halved BEFORE Armor is applied.
        * **Vulnerability**: Incoming damage of this type is doubled BEFORE Armor is applied.
* **Misc. Sidebar Values**
    * The rest of the values in the sidebar (Travel Speed, Travel Pace, Cost, Weight, etc.) are for flavor text only. These have no functionality.

### Configuration
Now that we've taken a look at some of the major stats, let's set up our player ship. Head on over to the **Configuration** tab by clicking the *Cog Wheel* icon in the tab navigation. You will see a sheet that looks like the following:

![](https://github.com/user-attachments/assets/963056d7-7136-41f2-8c80-825ee5235351)

**IMPORTANT**: note that various values, especially those inherited from Components, may not update until the **Full Reset** button is clicked on the top right corner: ![](https://github.com/user-attachments/assets/fcc2da8b-8d36-4a62-a46e-f47ff12a67d6).
...
4. **Component Inventory**: We'll next move on to defining the key starting characteristics of the ship via adding components to it. Add a component by either clicking the "+" in the top right corner of the section or creating a item of **Ship Component** and dragging it into the **Component Inventory** section. Open the ship component and you'll see a sheet like the following: ![](https://github.com/user-attachments/assets/4d75b425-a0a1-4785-b0bb-b24cdacea5ae)
    1. **Shields**: Start by going over to the **Details** tab changing the `Component Type` to `Shields`. You'll see several values; the following screenshot shows what I think are a good starting point, but feel free to adjust as necessary ![](https://github.com/user-attachments/assets/2ba827a4-4f3f-4b08-9965-f9ada051358b):
    ...
    2. **Armour**: Next, make a new ship component, hop over to the **Details** tab, and change the `Component Type` to `Armor`. You'll see something like the following screenshot; these are what I think are a good starting point ![](https://github.com/user-attachments/assets/22d6c66a-230c-4337-acab-bc8bf82476e0):
        * **Armor**: This is the starting/maximum armor for each given quadrant of the ship. Various effects can reduce these values, and upon resetting the ship's combat state, the armor values will default back to the configuration here.
            * Bow Armor also partially determines the damage a ship will deal and reduce reflected damage when *ramming* another ship. 
            * I like to make the armor values non-uniform to make positioning a bit more important.
        * **AC Contribution**: How much `Armor Class` is provided by the Armour component. The other half comes from the **Engine** component that we will go over next. I split this into two so that players can have a bit more nuance when deciding whether to swap out Armour and Engines.
    3. **Engine**: Next, make a new ship component, hop over to the **Details** tab, and change the `Component Type` to `Engine`. You'll see something like the following screenshot; these are what I think are a good starting point ![](https://github.com/user-attachments/assets/3b291ab2-5137-4a76-b32c-cc5c6c354284):
        ...
        * **AC Contribution**: How much `Armor Class` is provided by the Engine component. The other half comes from the **Armour** component that we went over previously. I split this into two so that players can have a bit more nuance when deciding whether to swap out Armour and Engines.
    4. **Sensor**: Next, make a new ship component, hop over to the **Details** tab, and change the `Component Type` to `Sensor`. You'll see something like the following screenshot; these are what I think are a good starting point ![](https://github.com/user-attachments/assets/8b70f6f6-c141-440c-a204-c989eedaca05):
        * **Hit Modifier**: Applies a flat modifier to hit for all weapons mounted to the ship. Think of this like a +N magic weapon bonus.
        * **Band Size**: Each weapon mounted to the ship will have a *Max Range*. Targets can be engaged outside of a weapon's max range, however, a stacking `-1` penalty will be applied depending on how many *bands* the target is outside of of the maximum range.
            ...
            * Targets can be engaged at most `Hit Modifier` bands beyond max range — i.e. once the stacking penalty would exceed the sensor's Hit Modifier, the target can no longer be engaged.
        * **Optimal Range**: The player in charge of the sensor usually needs to maintain various levels of lock for the gunner to be able to engage targets. Ships within optimal range automatically have at least *Lock Level 2* applied to them.
            * This is intended to be a distance that players can close to for an easy point blank engagement. It should be very small and similar to what we will later put for NPCs.
    ...
    5. **Reactor Core**: Next, make a new ship component, hop over to the **Details** tab, and change the `Component Type` to `Reactor Core`. You'll see something like the following screenshot; these are what I think are a good starting point ![](https://github.com/user-attachments/assets/fee820d8-f955-489e-84ae-9f8d9be19af1):
        ...
        * **Overclock Base DC**: The base DC for the Engineer's *Overclock* checks at minimum heat. The DC scales up by 10 as the reactor approaches maximum Heat.
            ...
            * I recommend staying around `10`, potentially moving this as low as `5` to as high as `15`, depending on how inconsistent you want this reactor to be. Remember, the max DC will be the base plus an additional `10`.
    6. **Ordnance Bay**: Next, make a new ship component, hop over to the **Details** tab, and change the `Component Type` to `Reactor Core`. Note that if you have `Strike Craft: No` under the **Configuration Tab**, you don't need to fill out the Strike Craft sections. You'll see something like the following screenshot; these are what I think are a good starting point ![](https://github.com/user-attachments/assets/51c8c355-2cff-428a-9682-f657192cd52f):  
...
5. **Weapons**: Next, we'll take a look at the weapons we want the ship to have. Go back to the **Configuration Tab** and adjust the **Weapon Battery Slots** accordingly to the number of weapons you want to have in each position. Each slot is centered on a different location:
    ...
    * Ordnance: The number of unique types of ordnance (strike craft and torpedoes) the ship will carry. More will allow the ship to be able to potentially fire multiple types of torpedoes or launch multiple types of strike craft.
    ...
    1. **Weapon Batteries**: To set up a weapon battery, make a new ship component, hop over to the **Details** tab, and change the `Component Type` to `Weapon Battery`. You'll see something like the following screenshot; this is an example weapon ![](https://github.com/user-attachments/assets/1dbd236e-7415-466d-ba11-5e660614cf08)
        ...
        * **Damage**: Damage for the weapon, entered as structured 5e dice (dice count × die size + flat bonus) plus a standard 5e **damage type** (fire, piercing, force, …). The damage type interacts with the target's native dnd5e Immunities / Resistances / Vulnerabilities.
    ...
    2. **Torpedoes**: To set up a torpedo, create a new **Starship** actor, choose **Ordnance** in the follow-up prompt, go to the **Configuration** tab, and under **Ordnance Type** select **Torpedo**. You'll see something like the following screenshot; this is an example configured weapon ![](https://github.com/user-attachments/assets/733c35ea-27ba-4a18-82f5-09a3600ec395):
        * **AC**: The Armor Class of the torpedo. Configure this by clicking the gear wheel in the AC shield in edit mode on the image UI and filling out a flat value in the resultant popup.
        ...
        * **WARHEAD TRAITS**: Rend, Armor Penetration, Shield Burn, and Shield Bypass. See the `Weapon Traits` section of the [README.md](README.md) for detailed descriptions.
    ...
    3. **Strike Craft**: To set up a strike craft, create a new **Starship** actor, choose **Ordnance** in the follow-up prompt, go to the **Configuration** tab, and under **Ordnance Type** select **Strike Craft**. You'll see something like the following screenshot; this is an example configured weapon ![](https://github.com/user-attachments/assets/580314d8-02bb-4b94-8aa7-628525d8bd74):
        * **AC**: The Armor Class of the strike craft. Configure this by clicking the gear wheel in the AC shield in edit mode on the image UI and filling out a flat value in the resultant popup.
        ...
        * **WEAPON TRAITS**: Rend, Armor Penetration, Shield Burn, and Shield Bypass. See the `Weapon Traits` section of the [README.md](README.md) for detailed descriptions.
...
6. **Register Components**: Now that you've set up all the components, updated the Max HP, Immunities, Resistances, and Vulnerabilities accordingly, you'll need to register/activate them. Head on over to the **Overview** tab, go to **Installed Components**, and make sure each item is set in the appropriate category/activated, as seen in the screenshot: ![](https://github.com/user-attachments/assets/a79c3630-4c3b-4637-aaa6-0bd36bd6b65a)
7. **Assign Player Roles**: Last step, assign roles! Go to the **Bridge Crew** section on the **Overview** tab and either click and drag player actors into each role, or allow them to assign the roles themselves! Players will have a button they can click to claim/release a given role. If you so wish, you can rename the role or adjust the Main Skill associated with each role as well.
...
To create an NPC ship, head on over to the Actors tab, create a new **Starship**, and choose **NPC Starship** in the follow-up prompt. You'll see a sheet like the one below:

![](https://github.com/user-attachments/assets/6534e05f-5741-4352-9238-f8ec3e139e7b)

Let's go over the parts:

### Portrait
* **Armor Class Badge**: Armor class is displayed here and can be configured in edit mode using the gear button.
* **Hit Points Bar**: Hit Points current can be edited here directly by double clicking; max can be configured in edit mode using the gear button.
* **Internal Fire**: Current internal fire of the ship. Can be edited directly by double clicking.
* **Initiative Button**: Current initiative modifier and button to roll initiative. Modifier is determined by the **PIL** value discussed below.

### Header
* **ARMOR CURRENT | ARMOR MAX**: Armor status overview.
    * Each of the four sections is broken into two numbers: the **current** armor value on the left, and the **maximum** on the right.
    * This section can be edited directly to change the current and maximum armor values.
* **SHIELDS**: A read-only display of the current shield status of each section on the ship.
* **Full Reset Button**: This button in the top right corner resets the combat state of the NPC ship.

### Ship Stats
NPC checks roll `1d20 + modifier` against the standard ship-action DC of 10, just like player ship checks.
...
* **GUN**: Gunnery modifier. A flat modifier to *Gunnery Checks* used to allocate additional *Accuracy*, *Penetration*, or *Firepower* points on the **Weapons** tab.
...
### Shield Allocation
This UI functions much the same way as the Player Ship's shield allocation UI, with the exception that the sector maxes are directly edited on the legend to the left and the max flux is directly edited on the legend to the right.

### Ship Conditions
Any active conditions affecting the ship will be listed here.

### Sidebar
Much of the miscellaneous stats of the ship are manually set here. These include **Immunities**, **Resistances**, and **Vulnerabilities**.
